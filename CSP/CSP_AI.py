...
class CSPAI(MoveStrategy):
    ...
    def get_move_from_guess(self, board) -> Tuple[int, int]:
        ...
        for x,y in get_full_ship_coords(self.best_guess, int(ship[1])):
            if board.map[y][x] == Grid.SPACE['empty']:
                self.prev_move = (x,y)
                return (x,y)
        print("HORRIBLE MISTAKE!!")
        return "THERE'S BEEN A HORRIBLE MISTAKE"
    def get_guess_from_solved(self, board):
        prev_result = self.get_prev_move_result(board)
    ...
    def get_next_unsunk_ship(self):
        for ship in VARIABLES:
            if ship not in self.sunk:
                return ship
        return None
    def handle_prev_move(self, board):
        prev_result = self.get_prev_move_result(board)
    ...
    def hunt(self, board):
        ...
        if prey_ship == None:
            ...
            # 3.b We have no room for shots
                ...
                self.hunt(board)
    def get_prey_ship(self, assignment, board):
    ...
    def handle_hunting_edge(self, board, ship, assignment):
        """
